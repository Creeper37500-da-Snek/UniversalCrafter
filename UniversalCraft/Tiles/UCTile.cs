...
namespace UniversalCraft.Tiles
{
	public class UCTile : GlobalTile
	{
		...
		Mod qolRecipes = ModLoader.GetMod("QualityOfLifeRecipes");
		Mod sacredTools = ModLoader.GetMod("SacredTools");
		Mod serrariaModTwo = ModLoader.GetMod("SerrariaModTwo");
		Mod simpleAutoChests = ModLoader.GetMod("SimpleAutoChests");
		...
		Mod touhouMod = ModLoader.GetMod("TouhouMod");
		...
		public override int[] AdjTiles(int type)
		{
			...
			if (type == mod.TileType("UniversalCrafter"))
			{
				...
				if (NPC.downedMoonlord)
				{
					...
					if (fargoSouls != null)
					{
						adjTile.Add(fargoSouls.TileType("CrucibleCosmosSheet"));
					}
				}
